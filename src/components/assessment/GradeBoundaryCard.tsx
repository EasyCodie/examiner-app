...
export const GradeBoundaryCard: React.FC<GradeBoundaryCardProps> = ({
  ...
  return (
    <div className="bg-[var(--cursor-canvas-soft)] border border-white/[0.08] rounded-xl p-5 relative overflow-hidden">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5 relative z-10">
        {/* Grade Badge */}
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-xl bg-[var(--cursor-primary)] flex flex-col items-center justify-center shrink-0 text-white shadow-sm">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-normal text-[var(--cursor-text-strong)] tracking-tight">{descriptor.title}</h2>
              <span className="px-2 py-0.5 rounded text-[10px] font-mono-code bg-[var(--cursor-surface-card)] text-[var(--cursor-text-body)] border border-white/[0.08]">
                Official 1–7 Scale
              </span>
            </div>
            <p className="text-xs text-[var(--cursor-text-body)] max-w-xl mt-1 leading-relaxed">
              {descriptor.desc}
            </p>
          </div>
        </div>
        ...
        {/* Score Stats */}
        <div className="flex items-center gap-4 bg-[var(--cursor-canvas)] border border-white/[0.08] px-4 py-3 rounded-lg">
          <div className="text-center pr-4 border-r border-white/[0.08]">
            <span className="text-[10px] font-mono-code text-[var(--cursor-text-muted)] block uppercase">Score</span>
            <div className="text-xl font-medium font-mono-code text-[var(--cursor-text-strong)]">
              {totalAwarded} <span className="text-xs text-[var(--cursor-text-muted)]">/ {totalPossible}</span>
            </div>
          </div>

          <div className="text-center pr-4 border-r border-white/[0.08]">
            <span className="text-[10px] font-mono-code text-[var(--cursor-text-muted)] block uppercase">Percentage</span>
            <div className="text-xl font-medium font-mono-code text-[var(--cursor-primary)]">
              {percentage}%
            </div>
          </div>
          ...
          {ecfCount > 0 && (
            <div className="text-center">
              <span className="text-[10px] font-mono-code text-amber-400 block uppercase flex items-center justify-center gap-1">
                <Sparkles className="w-3 h-3" /> ECF
              </span>
              <div className="text-lg font-medium font-mono-code text-amber-400">
                {ecfCount} <span className="text-xs text-[var(--cursor-text-muted)]">applied</span>
              </div>
            </div>
          )}
        </div>
      ...
      {nextBoundary && marksToNext > 0 && (
        <div className="mt-5 pt-3.5 border-t border-white/[0.06] flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2 text-[var(--cursor-text-body)]">
            <TrendingUp className="w-3.5 h-3.5 text-[#86efac]" />
            <span>
              Targeting <strong className="text-[var(--cursor-text-strong)]">Grade {nextGrade}</strong> ({nextBoundary}%):
              Need <strong className="text-[var(--cursor-primary)] font-mono-code">+{marksToNext} mark{marksToNext > 1 ? 's' : ''}</strong>.
            </span>
          </div>

          {/* Mini boundary scale */}
          <div className="flex items-center gap-1 font-mono-code text-[10px] text-[var(--cursor-text-muted)]">
            ...
            <span>•</span>
            <span className="text-[var(--cursor-primary)] font-semibold">G7: {boundaries.grade7}%</span>
          </div>
        </div>
      )}
    </div>
